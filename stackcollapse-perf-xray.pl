#!/usr/bin/perl -w
#
# stackcollapse-perf-xray.pl	collapse perf samples into single lines.
#
...
sub inline {
	...
	for (split /^/, $a2l_output) {
		...
		if ($one_item eq "") {
			...
			if ($show_context == 1) {
				push @fullfunc, $one_item . ":$_";
			} else {
				push @fullfunc, $one_item;
			}
			$one_item = "";
		}
	}
...
while (defined($_ = <>)) {
	...
	if (m/^$/) {
		...
		my $matched = any { /$ignore_pattern/ } @stack;
		my $len = scalar @stack;
		if (!$matched && $len > 1) {
			remember_stack(join("\n", @stack), 1) if @stack;
		}
